...
// 1 - generuojasi nuotraukos
// 2 - sugeneruotos nuotraukos objektas idedamas i kita masyva
// 3 - filtruoti pagal title
...
generateButton.addEventListener('click', function () {
  ...
  results.push(data[random]);
  output.textContent = '';
  generateAlbum(results);
});
...
filterInput.addEventListener('input', function (event) {
  ...
  if (results.length > 0) {
    const filterArray = results.filter(function (image) {
      // image.title === currentValue
      if (image.title.includes(currentValue)) {
        return true;
      } else {
        return false;
      }
    ...
    output.textContent = '';
    generateAlbum(filterArray);
  }
});

function generateAlbum(results) {
